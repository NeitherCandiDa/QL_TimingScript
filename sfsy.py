...
class Sfsy:
    ...
    async def login(self):
        ...
        if self.phone != "":
            log.log(f"用户【{self.phone}】 -  登录成功！✔️")
            return True
        else:
            log.log(f"用户【{self.phone}】 -  登录失败！❌")
            return False
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                if data.get("obj") and data.get("obj").get("integralTaskSignPackageVOList"):
                    ...
                    log.log(f">>> 用户【{self.phone}】 -  签到成功！✔️ - 获得【{reward_name}】， 本周累计签到{days}天")
                else:
                    log.log(f">>> 用户【{self.phone}】 -  今日已签到！✖️ - 本周累计签到{days + 1}天")
            else:
                log.log(f">>> 用户【{self.phone}】 -  签到失败！❌ - {data.get('errorMessage')}")
        else:
            log.log(">>> 用户【{self.phone}】 -  签到异常！‼️")
    ...
    async def super_welfare_benefit_sign_in(self):
        ...
        try:
            ...
            if data.get("success"):
                ...
                log.log(f">>> 用户【{self.phone}】 -  超值福利签到成功！✔️ - {status_msg} - 【{gift_name}】")
            else:
                error_message = data.get('errorMessage') or json.dumps(data) or '无返回'
                log.log(f">>> 用户【{self.phone}】 -  超值福利签到失败！❌ - {error_message}")
        except Exception as e:
            log.log(f">>> 用户【{self.phone}】 -  签到异常！‼️ - {e}")
    ...
    async def get_task_list(self, flag=False):
        ...
        if data.get("success") and data.get("obj") != []:
            ...
            if flag:
                log.log(f">>> 用户【{self.phone}】 -  当前积分：{total_point}")
    ...
    async def do_task(self):
        ...
        :return:
        """
        log.log(f">>> 用户【{self.phone}】 -  前往完成【{self.title}】任务...")
        response = await self.client.post(
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】完成成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】完成失败！❌ - {data.get('errorMessage')}")
    ...
    async def receive_task(self):
        ...
        :return:
        """
        log.log(f">>> 用户【{self.phone}】 -  前往领取{self.title}任务奖励...")
        response = await self.client.post(
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】奖励领取成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】奖励领取失败！❌ - {data.get('errorMessage')}")
    ...
    async def processe_tasks(self):
        ...
        for task in tasks:
            ...
            if status == 3:
                log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】 - 已完成！✅")
                continue
            if self.title in skip_keys:
                log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】 - 已跳过！♻️")
                continue
            else:
                await self.do_task()
    ...
    async def do_honey_task(self):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  蜂蜜任务【{self.task_type}】完成成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  蜂蜜任务【{self.task_type}】完成失败！❌ - {data.get('errorMessage')}")
    ...
    async def receive_honey_task(self):
        ...
        :return:
        """
        log.log(f">>> 用户【{self.phone}】 -  前往收取蜂蜜任务奖励...")
        self.headers.update(
            {
                "syscode": "MCS-MIMP-CORE",
                "channel": "wxwdsj",
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  收取蜂蜜任务【{self.task_type}】奖励成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  收取蜂蜜任务【{self.task_type}】奖励失败！❌ - {data.get('errorMessage')}")
    ...
    async def get_honey_task_list_and_start(self):
        ...
        if data.get("success"):
            for i in data["obj"]["list"]:
                ...
                status = i["status"]
                if status == 3:
                    log.log(f">>> 用户【{self.phone}】 -  任务【{self.task_type}】 - 已完成！✅")
    ...
    async def honey_damaoxian(self):
        ...
        for i in range(1, game_num):
            ...
            if data.get("success"):
                ...
                log.log(f"> 大冒险成功！剩余次数：{game_num}")
                await asyncio.sleep(2)
                game_num -= 1
            elif data.get("errorMessage") == "容量不足":
                log.log(f"> 大冒险失败！容量不足，需要扩容")
                await self.honey_expand()
            else:
                log.log(f"> 大冒险失败！❌ - {data.get('errorMessage')}")
    ...
    async def honey_expand(self):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  成功扩容【{data.get('obj')}】！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  扩容失败！❌ - {data.get('errorMessage')}")
    ...
    async def get_coupom(self):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  领取生活权益优惠券成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  领取生活权益优惠券失败！❌ - {data.get('errorMessage')}")
        ...
        if data.get("success"):
            ...
            for goods in goods_list:
                ...
                if exchange_times_limit >= 7:
                    ...
                    log.log(f">> 当前选择券号： {self.goodsNo}")
                    await self.get_coupom()
                    break
        else:
            log.log(f">>> 用户【{self.phone}】 -  获取生活权益优惠券列表失败！❌ - {data.get('errorMessage')}")

    async def honey_index_data(self, flag=False):
        if not flag: log.log(">>> 执行采蜜换大礼包...")
        ...
        if data.get("success"):
            ...
            if flag:
                log.log(f">>> 用户【{self.phone}】 -  当前蜂蜜：{usableHoney}")
                return
            ...
            if current_time.date() == activity_end_time.date():
                log.log(f"本期活动今日结束❗请及时兑换")
            else:
                log.log(f"本期活动结束时间 【{activity_end_time}】")
            if task_detail != []:
        ...
        if data.get("success"):
            ...
            for task in task_list:
                ...
                if "taskCode" in task:
                    ...
                    log.log(f">>> 用户【{self.phone}】 -  暂不支持【{self.title}】任务❗")
        await self.ear_end_2023_get_award()

    async def add_deliver_prefer(self):
        log.log(f">>> 开始【{self.title}】任务...")
        response = await self.client.post(
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  新增一个收件偏好成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】失败！❌ - {data.get('errorMessage')}")

    async def ear_end_2023_exchange_card(self):
        log.log(f">>> 开始积分兑换年卡任务...")
        response = await self.client.post(
        ...
        if data.get("success"):
            ...
            for card in received_account_list:
                log.log(f">>> 用户【{self.phone}】 -  兑换年卡成功！✅ - 获得【{card['urrency']}】卡【{card['amount']}】张！")
        else:
            log.log(f">>> 用户【{self.phone}】 -  任务【{self.title}】失败！❌ - {data.get('errorMessage')}")
        ...
        for index in range(10):
            for i in range(0, 3):
                ...
                if data.get("success"):
                    ...
                    log.log(f">>> 用户【{self.phone}】 -  用户账户余额不足！❌")
                    break
                elif data.get("errorMessage") == "用户信息失效，请退出重新进入":
                    log.log(f">>> 用户【{self.phone}】 -  用户信息失效，请退出重新进入❌")
                    break
                else:
                    log.log(f">>> 用户【{self.phone}】 -  抽取卡片失败！❌ - {data.get('errorMessage')}")
                    break
                await asyncio.sleep(3)
        ...
        if data.get("success"):
            ...
            if not current_account_list:
                ...
                for card in current_account_list:
                    ...
                    log.log(f">>> 用户【{self.phone}】 -  卡片【{currency_name}】 - 数量：{balance}")
            total_fortune_times = obj.get("totalFortuneTimes", 0)
            log.log(f">>> 用户【{self.phone}】 -  总卡片数量：{total_fortune_times}")
            return True
        else:
            log.log(f">>> 用户【{self.phone}】 -  查询卡片数量失败！❌ - {data.get('errorMessage')}")
            return False

    async def ear_end_2023_receive_task(self):
        log.log(f">>> 开始领取【{self.title}】任务奖励...")
        response = await self.client.post(
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  领取【{self.title}】任务奖励成功！✅")
        else:
            log.log(f">>> 用户【{self.phone}】 -  领取【{self.title}】任务奖励失败！❌ - {data.get('errorMessage')}")
    ...
    async def anniversary_2024_weekly_gift_status(self):
        ...
        if data.get("success"):
            ...
            log.log(f">>> 用户【{self.phone}】 -  查询每周领券失败！❌ - {error_message}")
    ...
    async def anniversary_2024_receive_weekly_gift(self):
        ...
        if data.get("success"):
            ...
            log.log(f">>> 用户【{self.phone}】 -  领取每周领券【{product_names}】成功！✅")
        else:
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">>> 用户【{self.phone}】 -  领取每周领券失败！❌ - {error_message}")
            if "系统繁忙" in error_message or "用户手机号校验未通过" in error_message:
                self.anniversary_black = True
    ...
    async def anniversary_2024_finish_task(self, task):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】 -  完成任务【{task['taskName']}】成功！✅")
            await self.anniversary_2024_fetch_mix_task_reward(task)
        else:
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">>> 用户【{self.phone}】 -  完成任务【{task['taskName']}】失败！❌ - {error_message}")
    ...
    async def anniversary_2024_fetch_mix_task_reward(self, task):
        ...
        if data.get("success"):
            ...
            log.log(f">> 领取任务【{task['taskName']}】奖励成功！✅ - 获得：{received_list}")
        else:
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 领取任务【{task['taskName']}】奖励失败！❌ - {error_message}")
    ...
    async def anniversary_2024_unbox(self):
        ...
        if data.get("success"):
            ...
            log.log(">> 拆盒子📦： %s" % ', '.join(unbox_list) or '空气')
        else:
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 拆盒子失败！❌ - {error_message}")
            if '用户手机号校验未通过' in error_message:
                self.anniversary_black = True
    ...
    async def anniversary_2024_game_list(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 查询游戏状态失败！❌ - {error_message}")
            if '用户手机号校验未通过' in error_message:
                self.anniversary_black = True
    ...
    async def anniversary_2024_search_word_win(self, index):
        ...
        try:
            ...
            if data.get("success"):
                ...
                error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
                log.log(f">>> 用户【{self.phone}】 -  找字游戏第{index}关失败！❌ - {error_message}")
                if '系统繁忙' in error_message:
                    flag = False
        except Exception as e:
            log.log(f">>> 用户【{self.phone}】 -  找字游戏异常‼️ - {e}")
            flag = False
        finally:
            return flag
    ...
    async def anniversary_2024_happy_elimination_win(self, index):
        ...
        try:
            ...
            if data.get("success"):
                ...
                log.log(f">>> 用户【{self.phone}】 -  第{index}关胜利！✅ - {rewards if rewards else '未获得奖励'}")
            else:
                error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
                log.log(f">>> 用户【{self.phone}】 -  第{index}关失败！❌ - {error_message}")
                if '系统繁忙' in error_message:
                    flag = False
        except Exception as e:
            log.log(f">>> 用户【{self.phone}】 -  第{index}关异常‼️ - {e}")
            flag = False
        finally:
            return flag
    ...
    async def anniversary_2024_topic_pk_choose_side(self, index):
        ...
        try:
            ...
            if data.get("success"):
                ...
                error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
                log.log(f">>> 用户【{self.phone}】 -  话题PK赛选择话题{index}失败！❌ - {error_message}")
                if '系统繁忙' in error_message:
                    flag = False
        except Exception as e:
            log.log(f">>> 用户【{self.phone}】 -  话题PK赛选择话题{index}异常‼️ - {e}")
            flag = False
        finally:
            return flag
    ...
    async def anniversary_2024_topic_pk_topic_list(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 查询话题PK赛记录失败！❌ - {error_message}")
    ...
    async def anniversary_2024_query_account_status_refresh(self):
        ...
        if not data.get("success"):
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 查询账户状态失败！❌ - {error_message}")
    ...
    async def anniversary_2024_title_list(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">> 查询每日口令竞猜失败！❌ - {error_message}")
    ...
    async def anniversary_2024_query_account_status(self):
        ...
        async def handle_error(message_prefix):
            ...
            log.log(f'{message_prefix}: {error_message}')
        ...
        if unbox_balance > 0:
            log.log(f'可以拆{unbox_balance}次盒子')
            # 如需实际拆盒，取消下方注释
        ...
        # 输出收集结果
        if card_collections:
            log.log(f'已收集拼图: {", ".join(card_collections)}')
        else:
            log.log('当前尚未收集到任何拼图')
    ...
    async def do_draw(self, cards):
        ...
        if data.get("success"):
            ...
            log.log(f">>> 用户【{self.phone}】 -  抽奖成功！✅ - 获得【{product_name}】")
            return True
        else:
            error_message = data.get('errorMessage') or json.dumps(response) or '无返回'
            log.log(f">>> 用户【{self.phone}】 -  抽奖失败！❌ - {error_message}")
            return False
    ...
    async def simulate_lottery(self, cards):
        while await self.can_draw(cards, 9):
            ...
            log.log(f">>> 用户【{self.phone}】，进行了一次9卡抽奖，消耗卡片: ", used_cards)
        while await self.can_draw(cards, 7) or await self.convert_common_card(cards, 'CARD_1'):
            ...
            log.log(f">>> 用户【{self.phone}】，进行了一次7卡抽奖，消耗卡片: ", used_cards)
        while await self.can_draw(cards, 5) or await self.convert_common_card(cards, 'CARD_1'):
            ...
            log.log(f">>> 用户【{self.phone}】，进行了一次5卡抽奖，消耗卡片: ", used_cards)
        while await self.can_draw(cards, 3) or await self.convert_common_card(cards, 'CARD_1'):
            ...
            log.log(f">>> 用户【{self.phone}】，进行了一次3卡抽奖，消耗卡片: ", used_cards)
    ...
    async def member_day_index(self):
        ...
        try:
            ...
            if data.get("success"):
                ...
                log.log(f">> 会员日可以抽奖{lottery_num}次")
                ...
                log.log(f">> 查询会员日失败！❌ - {error_message}")
    ...
    async def member_day_receive_invite_award(self, invite_user_id):
        ...
        if data.get("success"):
            ...
            log.log(f">> 会员日奖励： {product_name}")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 领取会员日奖励失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_lottery(self):
        ...
        if data.get("success"):
            product_name = data.get('obj', {}).get('productName', '空气')
            log.log(f">> 会员日抽奖成功！✅ - {product_name}")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 会员日抽奖失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_task_list(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询会员日任务列表失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_finish_task(self, task):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】，会员日任务【{task['taskName']}】完成！✅")
            await self.member_day_fetch_mix_task_reward(task)
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">>> 用户【{self.phone}】，会员日任务【{task['taskName']}】失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_fetch_mix_task_reward(self, task):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">>> 用户【{self.phone}】，会员日任务【{task['taskName']}】领取奖励成功！✅")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">>> 用户【{self.phone}】，会员日任务【{task['taskName']}】领取奖励失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_receive_red_packet(self, hour):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f">> 会员日领取{hour}点红包成功！✅")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 会员日领取{hour}点红包失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_red_packet_status(self):
        ...
        if data.get("success"):
            ...
            log.log(f"会员日合成列表： " + ", ".join(packet_summary))
            if self.member_day_red_packet_map.get(self.max_level):
                log.log(f"会员日已拥有[{self.max_level}级]X{self.member_day_red_packet_map.get(self.max_level)}")
                await self.member_day_packet_draw(self.max_level)
            else:
                remaining = self.packet_threshold - remaining_needed
                log.log(f"会员日距离[{self.max_level}级]红包还差： [1级]红包{remaining}")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询会员日红包状态失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_red_packet_merge(self, level):
        ...
        data = response.json()
        if data.get("success"):
            log.log(f"会员日合成: [{level}级]红包X2 -> [{level + 1}级]红包")
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 会员日合成两个[{level}级]红包失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def member_day_red_packet_draw(self, level):
        ...
        if data.get("success"):
            ...
            log.log(f"会员日提取[{level}级]红包: {', '.join(coupon_names) or '空气'}")
        else:
            error_message = data.get('errorMessage') if data else '无返回'
            log.log(f">> 会员日提取[{level}级]红包失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.member_day_black = True
                log.log(f"会员日任务风控‼️")
    ...
    async def midautumn_2024_index(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询活动状态失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
    ...
    async def midautumn_2024_game_index_info(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询中秋游戏信息！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
            return False
    ...
    async def midautumn_2024_game_init(self):
        ...
        if data.get("success"):
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 中秋游戏失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
            return False
    ...
    async def midautumn_2024_weekly_gift_status(self):
        ...
        if data.get("success"):
            ...
            for gift in obj:
                ...
                log.log(f">> 领取时间: {receiveStartTime} - {receiveEndTime}")
                if received:
                    log.log(f">> 该礼包已领取")
                    continue
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询每周礼包领取状态失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
    ...
    async def midautumn_2024_receive_weekly_gift(self):
        ...
        if data.get("success"):
            ...
            for gift in obj:
                ...
                amount = gift['amount']
                log.log(f">> 领取成功！✅ - {productName} X {amount}")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 领取每周礼包失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
    ...
    async def midautumn_2024_task_list(self):
        ...
        if data.get("success"):
            ...
            for task in obj:
                ...
                status = task['status']
                if status == 3:
                    log.log(f">> 任务【{self.task_name}】已完成！✅")
                    continue
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询推币任务列表失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
    ...
    async def midautumn_2024_coin_status(self, flag=False):
        ...
        if data.get("success"):
            ...
            if flag:
                if PUSH_TIMES_balance > 0:
                    for i in range(PUSH_TIMES_balance):
                        log.log(f'>> 开始第【{PUSH_TIMES_balance + 1}】次推币')
                        await self.midautumn_2024_push_coin()
                        PUSH_TIMES -= 1
                        pushedTimesToday += 1
                        pushedTimesTotal += 1
                log.log(f'> 剩余推币次数：【{PUSH_TIMES}】')
                log.log(f'> 当前金币：【{self.COIN_balance}】')
                log.log(f'> 今日推币：【{pushedTimesToday}】次')
                log.log(f'> 总推币：【{pushedTimesTotal}】次')
            else:
                log.log(f'> 剩余推币次数：【{PUSH_TIMES_balance}】')
                log.log(f'> 当前金币：【{self.COIN_balance}】')
                log.log(f'> 今日推币：【{pushedTimesToday}】次')
                log.log(f'> 总推币：【{pushedTimesTotal}】次')
            ...
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 查询金币信息失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
    ...
    async def midautumn_2024_push_coin(self):
        ...
        if data.get("success"):
            ...
            log.log(f">> 推币成功！✅ - 获得【{draw_award}】金币🪙")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 推币失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
    ...
    async def midautumn_2024_give_push_times(self):
        ...
        if data.get("success"):
            obj = data.get("obj", 0)
            log.log(f">> 领取成功！✅ - 获得【{obj}】推币次数")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">> 领取赠送推币次数失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")

    async def midautumn_2024_finish_task(self):
        log.log(f">>> 开始完成任务【{self.task_name}】...")
        response = await self.client.post(
            url='https://mcs-mimp-web.sf-express.com/mcs-mimp/commonPost/~memberEs~taskRecord~finishTask',
            headers=self.headers,
            json={
        ...
        if data.get("success"):
            ...
            log.log(f">>> 任务【{self.task_name}】完成成功！✅")
        else:
            error_message = data.get('errorMessage', '无返回')
            log.log(f">>> 任务【{self.task_name}】完成失败！❌ - {error_message}")
            if "没有资格参与活动" in error_message:
                self.midautumn_2024_black = True
                log.log(f"活动任务风控‼️")
            ...
            if data.get("success"):
                ...
                if current_award_list != []:
                    for award in current_award_list:
                        ...
                        log.log(f'>> 获得【{currency}】X【{amount}】')
                else:
                    log.log(">> 本关无奖励")
            else:
                error_message = data.get('errorMessage', '无返回')
                log.log(f">> 闯关失败！❌ - {error_message}")
                if "没有资格参与活动" in error_message:
                    self.midautumn_2024_black = True
                    log.log(f"活动任务风控‼️")
