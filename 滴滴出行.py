...
class DiDi:
    ...
    async def sign_in(self):
        ...
        if sign_in_response.status_code == 200:
            ...
            log.log(sign_in_data)
            if sign_in_data["errno"] == 0:
                subsidy_amount = sign_in_data["data"]["subsidy_state"]["subsidy_amount"]
                log.log(f"用户【{0}】, ===今日签到成功，获得{subsidy_amount}福利金🪙🪙🪙===")
                return
            elif sign_in_data["errno"] == 40009:
                log.log(f"用户【{0}】, ===今日福利金已签到，无需重复签到！===")
                return
            else:
                log.log(f"用户【{0}】, ===签到失败, {sign_in_data}===")
        else:
            log.log(f"===签到请求异常, {sign_in_response}===")
    ...
    async def get_carve_up_action_id(self):
        ...
        if get_carve_up_action_id_response.status_code == 200:
            ...
            log.log(f"===获取瓜分活动ID请求异常, {get_carve_up_action_id_response.text}===")
        ...
        if apply_carve_up_action_response.status_code == 200:
            ...
            if apply_carve_up_action_data.get("errno") == 0:
                if apply_carve_up_action_data.get("data", {}).get("result"):
                    log.log(f"用户【{self.user_phone}】, ===报名明日瓜分福利金成功🎉===")
                    return
            elif apply_carve_up_action_data.get("errno") == 1003:
                log.log(f"用户【{self.user_phone}】, ===今日瓜分福利金已报名，无需重复报名！===")
                return
            else:
                log.log(f"用户【{self.user_phone}】, ===报名明日瓜分福利金失败, {apply_carve_up_action_data}===")
                return
        else:
            log.log(f"===报名明日瓜分福利金请求异常, {apply_carve_up_action_response.text}===")
        ...
        if complete_carve_up_action_response.status_code == 200:
            ...
            if complete_carve_up_action_data.get("errno") == 0:
                if complete_carve_up_action_data.get("data", {}).get("result"):
                    log.log(f"用户【{self.user_phone}】, ===完成今日打卡瓜分福利金成功🎉===")
                    return
            elif complete_carve_up_action_data.get("errno") == 1003:
                log.log(f"用户【{self.user_phone}】, ===今日瓜分福利金已完成，无需重复完成！===")
                return
            else:
                log.log(f"用户【{self.user_phone}】, ===完成今日瓜分福利金失败, {complete_carve_up_action_data}===")
                return
        else:
            log.log(f"===完成今日瓜分福利金请求异常, {complete_carve_up_action_response.text}===")
    ...
    async def inquire_benefits_details(self):
        ...
        if benefits_details_response.status_code == 200:
            ...
            log.log(f"===查询权益详情请求异常, {benefits_details_response.text}===")
    ...
    async def receive_level_gift_week(self):
        ...
        for privilege in privileges_list:
            ...
            for coupon in coupons_list:
                ...
                log.log(f"用户【“{self.user_phone}】, ===开始领取{coupon.get('remark')}{coupon.get('coupon_title')}===")
                ...
                if receive_level_gift_response.status_code == 200:
                    ...
                    if receive_level_gift_data.get("errno") == 0:
                        log.log(f"用户【“{self.user_phone}】, ===领取成功🎉===")
                        continue
                    else:
                        log.log(f"用户【“{self.user_phone}】, ===领取失败, {receive_level_gift_data}===")
                else:
                    log.log(f"===领取周周领券请求异常, {receive_level_gift_response.text}===")
    ...
    async def receive_level_gift_month(self):
        ...
        """
        if not MONTH_SIGNAL:
            log.log(f"用户【“{self.user_phone}】, ===月月领券活动未开启===")
            return
        privileges_list = await self.inquire_benefits_details()
        for privilege in privileges_list:
            ...
            for coupon in coupons_list:
                ...
                if receive_level_gift_response.status_code == 200:
                    ...
                    log.log(f"===领取月月领券请求异常, {receive_level_gift_response.text}===")
                    ...
                    if swell_coupon_response.status_code == 200:
                        ...
                        if swell_coupon_data.get("errno") == 0:
                            if swell_coupon_data.get("data", {}).get("is_swell"):
                                log.log(f"用户【“{self.user_phone}】, ===膨胀成功🎉===")
                                continue
                            else:
                                log.log(f"用户【“{self.user_phone}】, ===膨胀失败, {swell_coupon_data}===")
                        else:
                            log.log(f"用户【“{self.user_phone}】, ===膨胀失败, {swell_coupon_data}===")
                    else:
                        log.log(f"===膨胀周周领券请求异常, {swell_coupon_response.text}===")
        ...
        for privilege in privileges_list:
            if privilege.get('name') == "行程意外险":
                ...
                if need_received == 1:  # 0为未领取，1为已领取
                    log.log(f"用户【“{self.user_phone}】, ===已经领取过了行程意外险===")
                    return
                elif need_received == 0:
                    log.log(f"用户【“{self.user_phone}】, ===开始领取行程意外险===")
                    ...
                    if receive_travel_insurance_response.status_code == 200:
                        ...
                        if receive_travel_insurance_data.get("errno") == 0:
                            log.log(f"用户【“{self.user_phone}】, ===领取行程意外险成功🎉===")
                        else:
                            log.log(
                                f"用户【“{self.user_phone}】, ===领取行程意外险失败, {receive_travel_insurance_data}===")
                    else:
                        log.log(f"===领取行程意外险请求异常, {receive_travel_insurance_response.text}===")
    ...
    async def receive_memberday_discount_multi(self):
        ...
        for privilege in privileges_list:
            if privilege.get('name') == "周三折上折":
                need_received = privilege.get('need_received')
                if need_received == 1:  # 0为未领取，1为已领取
                    log.log(f"用户【“{self.user_phone}】, ===已经领取过了周三折上折===")
                    return
                elif need_received == 0:
                    log.log(f"用户【“{self.user_phone}】, ===开始领取周三折上折===")
                    ...
                    if receive_memberday_discount_multi_response.status_code == 200:
                        ...
                        if receive_memberday_discount_multi_data.get("errno") == 0:
                            log.log(f"用户【“{self.user_phone}】, ===领取周三折上折成功🎉===")
                            return
                        else:
                            log.log(
                                f"用户【“{self.user_phone}】, ===领取周三折上折失败, {receive_memberday_discount_multi_data}===")
                    else:
                        log.log(f"===领取周三折上折请求异常, {receive_memberday_discount_multi_response.text}===")
        ...
        for bubble in bubble_list:
            if bubble.get('pre_content') == "完单返":
                ...
                if receive_wyc_order_finish_response.status_code == 200:
                    ...
                    log.log(f"===领取气泡奖励完单返福利金请求异常, {receive_wyc_order_finish_response.text}===")
    ...
    async def claim_coupon_check_in(self):
        ...
        if claim_coupon_check_in_response.status_code == 200:
            ...
            if claim_coupon_check_in_data.get("errno") == 0:
                ...
                log.log(f"用户【“{self.user_phone}】, ===领取天天神券签到失败, {claim_coupon_check_in_data}===")
        else:
            log.log(f"===领取天天神券签到请求异常, {claim_coupon_check_in_response.text}===")
        ...
        if get_draw_times_response.status_code == 200:
            ...
            for _ in range(lottery_chance):
                ...
                if lucky_draw_data.get("errno") == 0:
                    ...
                    log.log(f"用户【“{self.user_phone}】, ===天天神券抽奖失败, {lucky_draw_data}===")
                    return
        else:
            log.log(f"===天天神券获取抽奖次数请求异常, {get_draw_times_response.text}===")
    ...
    async def run_scratch(self):
        ...
        if await self.get_carve_up_action_id():
            log.log(f"用户【“{self.user_phone}】, ===开始完成今日瓜分活动===")
            if self.status_today == 2:
                await self.complete_carve_up_action()
            elif self.status_today == 3:
                log.log(f"用户【“{self.user_phone}】, ===今日瓜分活动已完成，无需重复完成！===")
            elif self.status_today == 4:
                log.log(f"用户【“{self.user_phone}】, ===今日已领取瓜分活动奖励！===")
            else:
                log.log(f"用户【“{self.user_phone}】, ===今日瓜分活动完成失败！肯昨日未报名！===")
            log.log(f"用户【“{self.user_phone}】, ===开始报名明日瓜分活动===")
            if self.status_tomorrow == 1:
                await self.apply_carve_up_action()
            elif self.status_tomorrow == 2:
                log.log(f"用户【“{self.user_phone}】, ===明日瓜分活动已报名，无需重复报名！===")
            else:
                log.log(f"用户【“{self.user_phone}】, ===明日瓜分活动报名失败！===")
        ...
        if get_batch_config_response.status_code == 200:
            ...
            for activity in activity_list:
                if activity.get("dchn") == "gL3E8qZ":
                    log.log(f"用户【“{self.user_phone}】, ===开始领取每日精选===")
                    ...
                    for coupon_index, coupon in enumerate(coupons_list):
                        ...
                        log.log(f"==={coupon_index + 1}.券名：{coupons_name} "
                                 f"状态：{coupons_status_name_dict.get(str(coupons_status))}===")
                        if coupons_status == 1:
                            log.log(f"===开始领取券：{coupons_name}===")
                            activity_id = coupon.get("activity_id")
                            if coupons_name == "打车5元券":
                                log.log(f"用户【{self.user_phone}】, ===【打车5元券】为分享助力才能领券，不支持自动领券===")
                                continue
                            ...
                            if bind_coupon_data.get("errno") == 0:
                                ...
                                log.log(f"用户【{self.user_phone}】, ===领取失败，{bind_coupon_data}===")
                                return
                if activity.get("dchn") == "kkXgpzO":
                    log.log(f"用户【“{self.user_phone}】, ===开始领取限时抢===")
                    ...
                    for seckill in seckill_list:
                        ...
                        log.log(f"☆☆场次：{seckill_name} 状态：{seckill_status_name_dict[str(seckill_status)]}")
            ...
            log.log(f"===每日精选请求异常， {get_batch_config_response.text}===")

    async def run(self):
        await self.get_user_info()
        log.log(f"用户【{self.user_phone}】, ===当前福利金数量为：{await self.get_welfare_payments()}===")
        task = [
            self.today_pick(),
            self.sign_in(),
            self.run_scratch(),
